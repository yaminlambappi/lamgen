...
class ValidationFramework:
    ...
    def _validate_string(self, value: Any, rule: ValidationRule, field_name: str) -> List[str]:
        ...
        if rule.allowed_values and value not in rule.allowed_values:
            allowed_values_str = ", ".join(map(str, rule.allowed_values))
            errors.append(f"{field_name} must be one of: {allowed_values_str}")
    ...
    def _validate_file(self, value: Any, rule: ValidationRule, field_name: str) -> List[str]:
        ...
        from apps.core.utils import validate_mime
        errors = []
        ...
        if rule.allowed_values:
            is_valid, detected_mime = validate_mime(value, rule.allowed_values[0])
            if not is_valid:
                errors.append(f"File type ({detected_mime}) not allowed. Allowed types: {', '.join(rule.allowed_values)}")
